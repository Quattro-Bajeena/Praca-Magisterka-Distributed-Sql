...
public class DescendingIndexTest : SqlTest
{
    ...
    {
        using DbCommand cmd = connection.CreateCommand();
        cmd.CommandText = "CREATE TABLE desc_idx_test (id INT PRIMARY KEY, value INT)";
        cmd.ExecuteNonQuery();

        cmd.CommandText = "CREATE INDEX idx_value_desc ON desc_idx_test (value DESC)";
        cmd.ExecuteNonQuery();
    ...
    protected override void ExecutePg(DbConnection connection, DbConnection connectionSecond)
    {
        ...
        using (DbDataReader reader = cmd.ExecuteReader())
        {
            ...
            while (reader.Read())
            {
                string? planText = reader.GetValue(0)?.ToString();
                if (planText != null && (planText.Contains("idx_value_desc") || planText.Contains("Index")))
                {
                    foundIndex = true;
                    break;
                }
    ...
    {
        using DbCommand cmd = connection.CreateCommand();
        cmd.CommandText = "DROP TABLE IF EXISTS desc_idx_test CASCADE";
        cmd.ExecuteNonQuery();
    }
}
